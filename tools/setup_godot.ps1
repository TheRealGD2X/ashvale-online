...
function Unzip($zipName, $dest) {
  $zip = Get-ChildItem -Path $dl -Filter $zipName | Select-Object -First 1
  if (-not $zip) { Write-Host "  missing: $zipName (in Downloads)" -ForegroundColor Yellow; return $null }
  $out = Join-Path $tmp $dest
  if (-not (Test-Path $out)) { Write-Host "  unpacking $($zip.Name)..."; Expand-Archive -Path $zip.FullName -DestinationPath $out -Force }
  return $out
}
...
if (-not (Test-Path (Join-Path $gd "Godot_v4.7.2-stable_win64.exe"))) {
  ...
  if ($z) { Expand-Archive -Path $z.FullName -DestinationPath $gd -Force; Write-Host "  ok" } else { Write-Host "  missing Godot zip in Downloads" -ForegroundColor Yellow }
...
foreach ($k in $kits) {
  ...
  $g = Get-ChildItem -Path $src -Recurse -Directory -Filter "glTF" | Where-Object { (Get-ChildItem $_.FullName -Filter *.gltf).Count -gt 0 } | Select-Object -First 1
...
foreach ($a in @(@{ zip = "Universal Animation Library*Standard*.zip"; glb = "UAL1_Standard.glb"; out = "UAL1.glb" }, @{ zip = "Universal Animation Library 2*Standard*.zip"; glb = "UAL2_Standard.glb"; out = "UAL2.glb" })) {
  ...
  if (-not $src) { continue }
  $f = Get-ChildItem -Path $src -Recurse -Filter $a.glb | Select-Object -First 1
  Copy-Item $f.FullName (Join-Path $anims $a.out) -Force
  Write-Host "  $($a.out)"
}
