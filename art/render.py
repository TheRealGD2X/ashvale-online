...
def load_job(path):
    ...
    base = {"size": 256, "ortho": 4.9, "shift": 0.22, "dirs": 8, "render_dirs": None, "samples": 8, "kind": "character",
            "hide": [], "parts": None, "sun": [28, 0, 0], "key": 3.6, "fill": 1.4, "ambient": 1.0, "shadow": False, "tint": None}
...
def setup_scene(job):
    ...
    sc.cycles.use_denoising = not job.get('_quick'); sc.cycles.use_adaptive_sampling = True
        ...
        if job.get('weapon_object'):
            ...
            for w in wobjs:
                if w.type == 'MESH':
                    ...
                    w.rotation_euler = Euler([math.radians(a) for a in job.get('weapon_rot', (0, 0, 0))])
    ...
    for aname, a in anims.items():
        ...
        for d in dirs:
            ...
            for i, f in enumerate(frames):
                ...
                meta['frames'].append({"anim": aname, "dir": d, "i": i, "file": fn}); n += 1
